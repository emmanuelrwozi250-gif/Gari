...
import {
  MapPin, Gauge, Calendar, BadgeCheck,
...
import type { ListingDisplay } from '@/app/(public)/buy/[id]/page';
...
export function BuyListingDetail({ listing }: Props) {
  ...
  const waText = encodeURIComponent(
    `Hi ${listing.seller.name}, I saw your ${listing.year} ${listing.make} ${listing.model} on Gari (ID: ${listing.id}) and I'm interested.`
  );
  ...
  async function submitEnquiry(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`/api/sales/${listing.id}/enquiry`, {
        method: 'POST',
          ...
          <div className="space-y-4">
            <div className="card p-6 lg:sticky lg:top-24">
              ...
              <p className="mt-3 text-xs text-text-light text-center">
                <button
                  onClick={() => toast('Report submitted. Our team will review it.', { icon: '⚑' })}
